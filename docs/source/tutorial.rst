...
.. currentmodule:: heterogeneoushmm

``heterogeneoushmm`` implements the Hidden Markov Models (HMMs).
...
- Problem 1: given the observed sequence :math:`\mathbf{X}`, which is the probability of that observed sequence for our model's parameters :math:`\boldsymbol{\theta}`, that is, which is p(:math:`\mathbf{X}` | :math:`\boldsymbol{\theta}`)? 
- Problem 2: given the observed sequence :math:`\mathbf{X}` and the model's parameters *θ*, which is the optimal state sequence :math:`\mathbf{S}`?
- Problem 3: which are the optimal :math:`\boldsymbol{\theta}` that maximizes p(:math:`\mathbf{X}` | :math:`\boldsymbol{\theta}`)?

The first and the second problem can be solved by the dynamic programming
algorithms known as the Viterbi algorithm and the Forward-Backward algorithm, respectively.
...
>>> from heterogeneoushmm.utils import pretty_print_hmm
...
>>> from heterogeneoushmm.utils import save_model, load_model
