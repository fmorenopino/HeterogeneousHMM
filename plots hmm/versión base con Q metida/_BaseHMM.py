...
import numpy as np
import math
from continuous.stats_struct import  stats_struct
...
class _BaseHMM(object):
    ...
    def forwardbackward(self,observations, cache=False):
        ...
        (alpha, tmp) = self._calcalpha(observations)
    ...
    def _calcalpha(self,observations):
        ...
        normalization_coefficients = np.zeros((len(observations)),dtype=self.precision)
        
        # init stage - alpha_1(x) = pi(x)b_x(O1)
        for x in xrange(self.n):
            alpha[0][x] = self.pi[x]*self.B_map[x][0]

        sum_tmp = np.sum(alpha[0])
        ...
        for t in xrange(1,len(observations)):
            ...
            sum_tmp = np.sum(alpha[t])
            alpha[t] = alpha[t] / sum_tmp #We NORMALIZE alpha to avoid overflow problems
            normalization_coefficients[t] = sum_tmp
                
        return (alpha, normalization_coefficients)

    def _calcbeta(self,observations, normalization_coefficients):
        '''
        Calculates 'beta' the backward variable.
        
        The beta variable is a np array indexed by time, then state (TxN).
        ...
        for t in xrange(len(observations)-2,-1,-1):
            ...
            beta[t] = beta[t] / normalization_coefficients[t]
    ...
    def _calcxi(self,observations,normalization_coefficients, alpha=None,beta=None):
        '''
        ...
        for t in xrange(len(observations)-1):
            ...
            for i in xrange(self.n):
                for j in xrange(self.n):
                    ...
                    tmp *= alpha[t][i]
                    tmp *= self.A[i][j]
                    tmp *= self.B_map[j][t+1] #B_map es la variable, mientras que _mapB es la función (implementada en _ContinuousHMM
                    tmp *= beta[t+1][j]
    ...
    def _calcgamma(self,xi,seqlen):
        ...
        Gamma is a (TxN) np array, where gamma[t][i] = the probability of being
    ...
    def train(self, observations, iterations=1,epsilon=0.0001,thres=(0.0015/100)): #poner el número en tanto por ciento
        ...
        for i in xrange(iterations):
            ...
            if(i == 0):
                stats_t_t_1 = stats_struct()  # On the first iteration the variable will be initialized


            prob_old, prob_new = self.trainiter(observations, stats_t_t_1)
    ...
    def Q_computation(self, stats, new_model, observations):
        ...
        A = 0
        for i in xrange(self.n):
            A += stats['gamma'][0][i] * new_model['pi'][i]
        ...
        for i in xrange(self.n):
            for t in xrange(len(observations)):
                C += stats['gamma'][t][i] * self._pdf(0, new_model['means'][i][0], new_model['covars'][i][0])



        return (A + B + C)
                
    def trainiter(self,observations, stats_t_t_1):
        '''
        ...
        if (stats_t_t_1.first_iteration == True):
            stats_t_t_1.firstIteration(stats)
            return (0,0)
        else:
            stats_t_t_1.updateStatsStruct(stats)
            # calculate the log likelihood of the previous model

            Q_old = self.Q_computation(stats_t_t_1.stats_t_1, new_model, observations)  # Actually, "new_model" has not been updated with the new parameters (next step)
            ...
            Q_new = self.Q_computation(stats_t_t_1.stats_t, new_model, observations)
    ...
    def M_stepA(self,observations,xi,gamma):
        '''
    ...
    def M_step(self,stats,observations):
        ...
        new_model['A'] = self.M_stepA(observations,stats['xi'],stats['gamma'])
    ...
    def _mapB(self,observations):
        ...
        as mentioned above. 'self.B_map' is an (TxN) np array.
